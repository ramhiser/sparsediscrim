...
#'
#' Given a set of training data, this function builds the RSDDA classifier. To
#' improve the estimation of the pooled variances, we use a shrinkage method
#' from Pang et al. (2009).  This yields the SDQDA classifier.
#'
#' The RSDDA classifier is a regularized version of SDQDA, where the
#' off-diagonal elements of the pooled sample covariance matrix are set to zero.
#'
#' @export
#'
...
rsdda <- function(x, y, num_alphas = 101) {
	...
	estimators <- dlply(obj$training, .(labels), function(df_k) {
		...
		var_shrink <- var_shrinkage(N = n_k, K = 1, var_feature = var,
                                num_alphas = num_alphas, t = -1)
		list(xbar = xbar, var_k = var_shrink, sum_squares = sum_squares, n = n_k,
         pi_k = pi_k)
	})
	
	var_pool <- colSums(laply(estimators, function(class_est) {
    class_est$sum_squares
    })) / N
	var_pool_shrink <- var_shrinkage(N = N, K = obj$num_classes,
                                   var_feature = var_pool,
                                   num_alphas = num_alphas, t = -1)
...
#'
#' Estimates the optimal value of lambda used in RSDDA by leave-k-out
#' crossvalidation.
#'
...
#' estimation of the pooled variances, we use a shrinkage method from Pang et
#' al. (2009).  This yields the SDQDA classifier.
#'
#' The RSDDA classifier is a regularized version of SDQDA, where the
#' off-diagonal elements of the pooled sample covariance matrix are set to zero.
#' 
...
#' @param grid_size the grid size of lambda_grid considered in estimating the
#' regularization parameter lambda
#' @param k Leave-k-out crossvalidation is used to estimate the optimal value of
#' lambda.
#'
#' @references Dudoit, S., Fridlyand, J., & Speed, T. P. (2002). "Comparison of
#' Discrimination Methods for the Classification of Tumors Using Gene Expression
...
model.select.rsdda <- function(object, grid_size = 5, k = 1) {
	...
	if(k > 1) {
		k <- 1
		warning("Leave-k-out crossvalidation for k > 1 is not implemented yet.
             Using k = 1 instead...\n")
	}
...
predict.rsdda <- function(object, newdata, num_lambdas = 5, lambda = NULL,
                          verbose = FALSE) {
	if (!inherits(object, "rsdda"))  {
		stop("object not of class 'rsdda'")
	}
	...
	predictions <- apply(newdata, 1, function(obs) {
		scores <- sapply(object$estimators, function(class_est) {
			var_rsdda <- (class_est$var_k)^(1 - object$lambda) *
        (class_est$var_pool)^(object$lambda)
			sum((obs - class_est$xbar)^2 * var_rsdda) - sum(log(var_rsdda))
        - 2 * log(class_est$pi_k)
		})
